...
describe("Movie E2E", () => {
    ...
    it("should find movie by id", async () => {
        ...
        const created = await request(app).post("/movie").send(movie).expect(201);

        const response = await request(app).get(`/movie/${created.body.idMovie}`).expect(200);

        assert.equal(response.body.idMovie, created.body.idMovie);
    ...
    it("should update movie", async () => {
        ...
        assert.equal(response.body.idMovie, created.body.idMovie);
        assert.equal(response.body.title, "Novo Filme");
    });
    ...
    it("should delete movie", async () => {
        ...
        await request(app).delete(`/movie/${created.body.idMovie}`).expect(204);
