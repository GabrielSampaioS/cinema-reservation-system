...
describe("Room E2E", () => {
    it("should create a room", async () => {
        ...
        const response = await request(app).post("/room").send(room).expect(201);
    ...
    it("should find room by id", async () => {
        ...
        const response = await request(app).get(`/room/${createdRoom.body.idRoom}`).expect(200);

        assert.equal(response.body.idRoom, createdRoom.body.idRoom);
    ...
    it("should update room", async () => {
        ...
        assert.equal(response.body.name, "Sala Premium");
    ...
    it("should delete room", async () => {
        ...
        await request(app).delete(`/room/${createdRoom.body.idRoom}`).expect(204);
    });
});
