...
async function createRoom() {
    ...
    const createdTheatre = await request(app).post("/theatre").send(theatre).expect(201);

    const room = makeRoomData({
        theatreId: createdTheatre.body.idTheatre,
    });

    const createdRoom = await request(app).post("/room").send(room).expect(201);
...
describe("Session E2E", () => {
    ...
    it("should list all sessions", async () => {
        ...
        const session = makeSessionData({ movieId, roomId });

        await request(app).post("/session").send(session).expect(201);
        ...
        assert.equal(response.body[0].movieId, movieId);
    });
    ...
    it("should find session by id", async () => {
        ...
        const created = await request(app).post("/session").send(session).expect(201);

        const response = await request(app).get(`/session/${created.body.idSession}`).expect(200);

        assert.equal(response.body.idSession, created.body.idSession);
    });
    ...
    it("should delete session", async () => {
        ...
        await request(app).delete(`/session/${created.body.idSession}`).expect(204);
    });
    ...
    it("should list sessions by movie", async () => {
        ...
        const response = await request(app).get(`/session/movie/${movieId}`).expect(200);
